globals [
  ...
  crowded-patch 
  crowd-level    ;; patch where we show the "CROWDED" label
]
...
to go
  ...
  set attendance count turtles-on bar-patches  if attendance > overcrowding-threshold [
    ask crowded-patch [ set plabel "CROWDED"  set crowd-level crowd-level + 1  ]
  ]
